...
async def okgoogle(img):
    ...
    if message and message.media:
        ...
        await img.edit("`Replica foto o sticker.`")
        return

    if photo:
        await img.edit("`Carico...`")
        try:
            image = Image.open(photo)
        except OSError:
            await img.edit('`Sessualità non supportata, molto probabilmente.`')
            return
        name = "okgoogle.png"
        image.save(name, "PNG")
        image.close()
        ...
        if response != 400:
            await img.edit("`Imagina caricata su google..`"
                           "\n`Analisi della fonte ora..`")
        else:
            await img.edit("`Google mi ha detto di andare a fanculo.`")
            return
...
CMD_HELP.update({
    'reverse':
    '.reverse\
        \nUsa: Rispondi a una foto/adesivo per eseguire una ricerca inversa su Google Immagini !!'
})

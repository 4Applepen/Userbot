...
async def _(event):
    ...
    if input_str in (("bots", "commands", "email", "forward", "url")):
        ...
        try:
            ...
            await event.edit(
                "Le autorizzazioni predefinite della chat corrente sono state modificate correttamente, nell'API"
            )
...
async def _(event):
    ...
    if not current_db_locks:
        ...
        res = "Di seguito sono riportati i blocchi di DataBase in questa chat: \n"
    ...
    try:
        ...
        res += "\nDi seguito sono riportati i blocchi API in questa chat: \n"
    ...
    if is_locked(peer_id, "commands"):
        ...
        if is_command:
            try:
                ...
                await event.reply(
                    "Non mi sembra di avere l'autorizzazione ADMIN qui. \n`{}`".format(str(e))
                )
        ...
        if event.user_added:
            ...
            if Config.G_BAN_LOGGER_GROUP is not None and is_ban_able:
                ban_reason_msg = await event.reply(
                    "!warn [user](tg://user?id={}) non aggiungere bot qua.".format(users_added_by)
                )

...
async def _(event):
    ...
    if "pornhub" not in d_link:
        await event.edit("` Ho bisogno del link.`**(._.)**")
    else:
        await event.edit("**💦Preparo il video scaricato💦 **\n**Title**:  `{}`\n**Total Views**: `{}`".format(title, views))
    ...
    async with event.client.conversation("@phsavebot") as conv:
          try:
              ...
              if "language" in oop.text:
                  await borg.send_message(event.chat_id, "**Perfavore vai su** @phsavebot ** e seleziona la tua lingua**")
              await asyncio.sleep(2)
              ...
              if "Downloading" in response.text:
                  ...
                  await borg.send_file(event.chat_id, video_hehe, caption="`🤤 video hostato da` [@sqdboyuwu](https://t.me/sqdboyuwu)!🤤\n**Title:** `{}`".format(title))
              elif "Unfortunately" in response.text:
                  await event.edit("`Link non corretto\n Riprova.**")
              elif "correct" in response.text:
                  await borg.send_message(event.chat_id, response.text)
          except YouBlockedUserError: 
              await event.reply("**Perfavore sblocca @phsavebot**")
              return

...
async def _(event):
    HELP_STR = "`.rmbg` Usalo via reply"
    if event.fwd_from:
        return
    if Config.REM_BG_API_KEY is None:
        await event.edit("Ti serve l'api per usarlo.")
        return False
    ...
    if event.reply_to_msg_id:
        ...
        await event.edit("`Elaboro la foto.`")
        try:
            ...
            await event.edit("Mando a ReMove.BG")
    ...
    if "image" in contentType:
        ...
        await event.edit("Background rimosso in {} secondi, powered by @sqdboyuwu".format(ms))
    else:
        await event.edit("ReMove.BG API errors\n`{}".format(output_file_name.content.decode("UTF-8")))

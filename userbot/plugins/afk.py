...
async def _(event):
    ...
    if not USER_AFK:  # pylint:disable=E0602
        ...
        if reason:
            await borg.send_message(event.chat_id, f"**Vado afk!** __Motivo ~ {reason}__")
        else:
...
async def on_afk(event):
    ...
    if USER_AFK and not (await event.get_sender()).bot:  # pylint:disable=E0602
        ...
        msg = None
        message_to_reply = f"__Il mio capo è afk da__ `{total_afk_time}`\nDove si trova: ~~ SOLO DIO LO SA~~ " + \
            f"\n\n__Lascia un messaggio per ora.__\n**Motivo**: {reason}" \
            if reason \
            else f"**Heya!**\n__Non sono disponibile da {total_afk_time} Quando torno ti rispondo.**( ಠ ʖ̯ ಠ)**  "

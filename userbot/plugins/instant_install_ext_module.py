...
async def install(event):
    ...
    for ixo in total_doxx:
        ...
        if "(" not in downloaded_file_name:
            ...
            await borg.send_message(event.chat_id, "Plugin instato `{}` con successo.".format(os.path.basename(downloaded_file_name)))
        else:
            await borg.send_message(event.chat_id, "Plugin `{}` è già installato o pre installato..".format(os.path.basename(downloaded_file_name)))

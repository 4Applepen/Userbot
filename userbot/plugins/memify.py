...
async def _(event):
    ...
    if not event.reply_to_msg_id:
       await event.edit("`Sintassi: rispondi a un'immagine con .mms` 'testo sopra'; "testo in basso" ")
       return
    reply_message = await event.get_reply_message() 
    if not reply_message.media:
       await event.edit("```rispondi a un imagine/sticker/gif```")
       return
    chat = "@MemeAutobot"
    ...
    if reply_message.sender.bot:
       ...
       await event.edit("```Tempo di Trasfigurazione! Mwahaha memorizzo questa immagine! (」ﾟﾛﾟ)｣ ```")
    ...
    async with borg.conversation("@MemeAutobot") as bot_conv:
          try:
            ...
            response = await bot_conv.get_response()
              ...
              await event.reply("```Sblocca @MemeAutobot e riprova```")
              return
          if response.text.startswith("Forward"):
              await event.edit("```puoi gentilmente disabilitare le tue impostazioni sulla privacy in avanti per un buon nibba?```")
          if "Okay..." in response.text:
            await event.edit("```🤨 NANI?! Questa non è un'immagine! Questo richiederà la somma tym per convertire in immagine owo 🧐```")
            thumb = None
            ...
            await borg.send_file(  # pylint:disable=E0602
                ...
                requires_file_name,
                supports_streaming=False,
                caption="Userbot: Modificato da @sqdboyuwu",

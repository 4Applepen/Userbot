...
async def on_snip_save(event):
    ...
    if msg:
        ...
        await event.edit(f"filtro {name} salvato con successo. usalo con {name}")
    else:
        await event.edit("Rispondi a un messaggio con .savefilter parola per salvare il filtro")
...
async def on_snip_delete(event):
    ...
    await event.edit(f"filtro {name} cancellato")


@command(pattern="^.clearallfilters$")
async def on_all_snip_delete(event):
    remove_all_filters(event.chat_id)
    await event.edit(f"tutti i filtri sono stai cancellati.")
